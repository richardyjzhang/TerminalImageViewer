...
struct size
{
  ...
  size(cimg_library::CImg<unsigned int> img) : width(img.width()), height(img.height())
  {
  }
  unsigned int width;
  unsigned int height;
...
void emit_usage()
{
  std::cerr << "Terminal Image Viewer v1.1.0" << std::endl
            << std::endl;
  std::cerr << "usage: tiv [options] <image> [<image>...]" << std::endl
            << std::endl;
  ...
  std::cerr << "  -x        : Use new Unicode Teletext/legacy charactery (experimental)." << std::endl
            << std::endl;
}
